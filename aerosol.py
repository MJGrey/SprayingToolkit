...
class Aerosol:
    ...
    def load(self, loader):
        loader.add_option(
            name="target", typespec=str, default="", help="Target domain or URL"
        )

        loader.add_option(
            name="language", typespec=str, default="en", help="text language"
        )

        loader.add_option(
            name="aws_region", typespec=str, default="us-east-1", help="AWS region"
        )
        ...
        try:
            if "html" in flow.response.headers["Content-Type"] and len(
                ...
                if ctx.options.target in flow.request.host:
                    ...
                    ctx.log.info(
                        print_good(
                            f"Got {len(the_best_words)} words, the best words..."
                        )
                    )
            ...
            ctx.log.info(
                json.dumps(
                    self.comprehend.detect_key_phrases(
                        Text=text, LanguageCode=ctx.options.language
                    ),
                    sort_keys=True,
                    indent=4,
                )
