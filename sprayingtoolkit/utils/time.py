...
from sprayingtoolkit.utils.messages import print_info
...
def countdown_timer(hours, minutes, seconds, now=datetime.now):
    delay = timedelta(hours=int(hours), minutes=int(minutes), seconds=int(seconds))
    ...
    try:
        for remaining in range(int(delay.total_seconds()), 0, -1):
            ...
            print(
                print_info(
                    f"{timedelta(seconds=remaining - 1)} remaining until next spray"
                ),
                end="\r",
            )
