...
async def autodiscover_url_not_found_exception_handler(
    request: Request, exc: AutodiscoverUrlNotFound
):
    return JSONResponse(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"error": str(exc)},
    )


@api.exception_handler(LyncBaseUrlNotFound)
async def lync_base_url_not_found_exception_handler(
    request: Request, exc: LyncBaseUrlNotFound
):
    return JSONResponse(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"error": str(exc)},
    )
